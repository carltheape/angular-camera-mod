...
export class DataService {
    ...
    [
      ...
      (params: any, data: any) => {
        if (params && typeof params.id !== 'undefined') {
          const assignment = this.assignments.get(params.id);
          if (assignment) {
            this.assignments.set(params.id, { ...assignment, deleted: true });
          }
        }
        return {};
      }
  ...
  private assignments = new Map<number, Assignment>([
    ...
    [1, { cameraId: 1, vehicleId: 1, dateCreated: new Date(10), deleted: false }]
  ]);
